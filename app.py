...
def getPoke(poke_name):

    response = requests.get(f"https://pokeapi.co/api/v2/pokemon/{poke_name.lower()}")

    if response.status_code != 200:
        print(f"Error fetching data for {poke_name}! Status code: {response.status_code}")
        return None
